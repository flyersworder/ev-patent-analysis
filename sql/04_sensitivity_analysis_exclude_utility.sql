-- Query 4: Sensitivity Analysis - Exclude Chinese Utility Models
-- Purpose: Re-run main analysis excluding unexamined Chinese utility models
-- Compare results to identify impact of patent quality differences
...
patent_data AS (
  SELECT
    ...
    p.filing_date,
    p.country_code as patent_office,
    p.kind_code,
  ...
  WHERE p.filing_date IS NOT NULL
    ...
    -- EXCLUDE Chinese utility models (unexamined patents)
    -- Chinese utility model kind codes: U (main utility model), U8 (corrected), U9 (corrected)
    -- Keep only: A (invention application), B (invention grant), and their corrections
    AND NOT (p.country_code = 'CN' AND p.kind_code IN ('U', 'U8', 'U9'))
),
...
SELECT
  ...
  'Excluding Chinese Utility Models' as note
